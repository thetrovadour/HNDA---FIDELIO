...
### 2026-06-01 — Warm-up wrong answer no longer auto-starts the run (revises "click-to-start always commits")
**Decision:** `AttemptPuzzle`'s Warmup branch no longer commits unconditionally. A **wrong** warm-up answer now consumes the tile (`WithConsumed` + `TileConsumed`) and returns **without committing** — the totem does not advance and the run does not start. Only a **correct** answer commits (warm-up → Running) with the time bonus. Removed the now-orphaned `WrongPenaltySeconds` const (its only caller was the old warm-up else-branch; the Running wrong-path uses a hardcoded `ShoveMist(2)`). No warm-up game-over: the player picks another column-1 tile, or moves along column 0 to re-roll the frontier — you earn your start.
**Why:** The earlier "click-to-start: always commit, wrong = trolling tax" rule let a wrong first answer still launch the run on that tile, which read as a cheat (advance regardless). Cristian chose to fix the bug rather than paper over it with a freebie tile. Warm-up stays a safe staging area (no clock, no mist yet), so a wrong answer costs the tile but not the run. **Caveat flagged:** moving along column 0 during warm-up rebuilds `frontierMetadata` (fresh, un-consumed) — so warm-up consumption is escapable by re-rolling. That is acceptable for a pre-start staging phase; revisit if warm-up should enforce a 3-strike like Running.
**Phase:** G2 / C1.

### 2026-06-01 — C1 bugfix: wrong-consumed frontier tiles now render black (and repaint live)
**Decision:** Two rendering-layer fixes for wrong-answer feedback. (1) `GridWorld.GetCellView` returns `VisualState.Consumed` for any frontier tile whose `frontierMetadata` entry is `Consumed` — previously such a tile was still `IsChoosable` and rendered as `Revealed` (category color). (2) `GridRendererBehaviour` now subscribes to `Engine.TileConsumed` (→ `RefreshCell`) so the blacken repaints the instant a wrong answer lands. Added regression test `GetCellView_WrongConsumedFrontierTile_RendersConsumed`. Also migrated the two obsolete `ResolvePuzzle` test callers to `AttemptPuzzle` (cleared CS0618); the `[Obsolete] ResolvePuzzle` shim now has zero callers and is dead code (flagged, not deleted).
**Why:** C1 playtest reported "wrong answers don't blacken / advance no matter what / 3-strike never fires." Root cause was purely visual: the engine consumed tiles correctly (unit-tested) and the client sent correctness correctly (verified via `QuestionServe.was_correct` in the DB — 15 wrong / 4 right), but wrong answers produced zero on-screen feedback, so they felt like no-ops. Fixing the view + the missing event subscription makes the strike visible; the engine's `FrontierExhausted` game-over was always firing once all three are truly consumed.
**Phase:** G2 / C1.

### 2026-06-01 — C1 bugfix: consumed frontier tiles are no longer re-clickable
**Decision:** `InputAdapterBehaviour.HandleFrontierClick` now swallows clicks on any frontier tile whose `TileMetadata.Consumed == true` (guard added right after the `IsFrontier` check, before the Black-trap branch). A consumed tile presents no new question card and triggers no fetch.
**Why:** C1 playtest surfaced two symptoms that were one root cause — after a wrong answer the tile blackened (engine consumed it correctly) but the input layer still let the player re-click it, fetching a fresh question. That let the player retry the same tile until correct ("cheat by re-answering") and, as a consequence, the 3-strike `FrontierExhausted` game-over never fired (you never let all three tiles get consumed because you kept retrying one). The engine was already correct (`AttemptPuzzle` throws on a consumed tile; `AllFrontierConsumed → GameOver(FrontierExhausted)` is unit-tested). The fix belongs in `InputAdapter` because it is the input-policy layer ("what kind of click does what", per the B5 decision) — `QuestionFlow` stays unaware of trap/consumed state. Also hardened the fire-and-forget 404 path to log a calm warning (empty bucket is expected, not an error).
**Phase:** G2 / C1.

### 2026-06-01 — C1 seed coverage: 10 → 37 questions (zero 404 gaps across all 3 request paths)
**Decision:** Expanded `prisma/seed-questions.json` to 37 questions so every request path resolves: 30 MC cells (6 categories × 5 tiers, normal + wildcard paths) + 6 TF at tier 3 (one per category, TF-injection path). The original 10 left 20 of 30 MC cells empty plus TF/wildcard gaps; ~2/3 of frontier clicks would have hard-404'd because the client requests deterministic `(category, tier[, kind])` combos and the controlled-repeat fallback only repeats within a *non-empty* bucket.
**Why:** A meaningful end-to-end C1 needs every frontier click to resolve. This pulls a lean functional slice of C2 forward (placeholder-quality content, polished/expanded to the full 100 in the C2 content sprint). JWT minted for the existing `catr_test@hnda.local` user; all three paths (normal MC, TF-injection, wildcard random-category) live-verified over HTTP before handing to the Editor.
**Phase:** G2 / C1 (content slice; full 100 deferred to C2).

### 2026-06-01 — B6 shipped + verified: GridRenderer hint badges
**Decision:** `GridRendererBehaviour` carries a per-cell `Dictionary<GridCoord, TextMeshPro>` parallel to its sprite cells. `SpawnCell` creates a badge GameObject (fontSize 3, centered, sortingOrder 5). `RefreshBadge` reads `Engine.TryGetTileMetadata(c, out md)` and paints `T{md.Tier}` only when `md.HintVisible && !md.Consumed`, else clears. Badges therefore appear on frontier tiles only — `TryGetTileMetadata` returns false for any tile not in `frontierMetadata`. Verified live in the Editor: badges appear stochastically and their rate climbs with tier. No new code this session — B6 had landed in an earlier commit; this session was inspect + visual-verify + seal.
**Why:** Renderer stays a pure paintbrush — the hint probability (`0.20 + 0.15*(tier-1)`) lives in the engine's `RollHintVisible`, snapshotted into `frontierMetadata` at reveal so the hint is truthful and stable for the tile's life. Parallel badge dictionary keyed on the same `GridCoord` keeps badge lifecycle (spawn/remove on window rebuild) locked to the cell it annotates. `&& !md.Consumed` clears the badge the moment a tile is spent — no stale `T3` on a dead tile.
**Phase:** G2 / B6.

### 2026-05-27 — B5 shipped: InputAdapter rewire + GameBootstrap composition root
**Decision:** `InputAdapterBehaviour` gets `Bind(QuestionFlow)`. In Running phase, a frontier click now: (a) if the tile is `CellCategory.Black`, calls `AttemptPuzzle(target, correct: false, 0f)` directly — no question fetch, no card; (b) otherwise fires `_ = _flow.RequestQuestion(target)`. The G1 placeholder `ResolvePuzzle(correct: true)` is gone. A new `GameBootstrap` module owns composition: builds an 8-question in-memory pool (1 per category at varying tiers + 1 TF + 1 wildcard), constructs `MockBackendClient` and `QuestionFlow`, then injects into `QuestionUIController.Bind(flow, engine)` and `InputAdapterBehaviour.Bind(flow)` at `Start()`. Lang reads `Application.systemLanguage`; seed hardcoded 42 for B5.
**Why:** `InputAdapter` as the policy layer for "what kind of click does what" — including the Black-trap no-question path. Putting it in QuestionFlow would force the flow to know about trap tiles, which violates its module boundary (it only handles question fetch + resolve). `GameBootstrap` as a separate composition root keeps `InputAdapter` focused on input routing and `QuestionUIController` focused on UI; neither has to know how the flow is built. One swap-point when `HttpBackendClient` lands in C1.
**Phase:** G2 / B5.

### 2026-05-27 — B4 shipped: QuestionUI module (in-grid card prefab + controller)
**Decision:** New `QuestionUI/` asmdef. `QuestionCardBehaviour` is the prefab controller — World-Space Canvas (3×2 world units) parented to scene; `Show(ActiveQuestion, Vector3 worldPos)` positions, sets prompt, instantiates one button per answer from a prefab; click disables all buttons (server-first transparency); `Hide()` clears + deactivates. `QuestionUIController` is the scene glue — `Bind(QuestionFlow, GridWorld)` wires `QuestionFetched` (positions card 1.5 cells above clicked tile, flips below if `Y==0`), `AnswerResolved` (hides), and engine `GameOver` (hides — "no screenshots allowed"). `CellSize`, `RowCount`, `GridToWorld` exposed on `GridRendererBehaviour` for card placement. `QuestionFlow.TfTier`/`CadenceMin`/`CadenceMax`/`TfCapMin`/`TfCapMax` promoted from `internal` to `public` so Unity-separated test assembly can read them.
**Why:** World-Space canvas (not Screen-Space) so the card lives *in the grid* and rides camera follow. Position-once at `QuestionFetched` is sufficient because the clicked tile doesn't move during the question. Disabling buttons on click instead of dismissing the card honors the server-first invariant — the player sees the resolution land before the UI moves. `internal` → `public` because Unity assemblies don't share internals across asmdef boundaries (sidecar `dotnet test` ran in a single assembly, masking the gap).
**Phase:** G2 / B4.

### 2026-05-26 — B2 shipped: BackendClient module (IBackendClient + DTOs + Mock + Http)
**Decision:** Two asmdefs mirroring the GridEngine split — pure-C# `BackendClient` (interface, DTOs, `MockBackendClient`, `MockPlayerSession`) and Unity-deps `BackendClient.Unity` (`HttpBackendClient` via `UnityWebRequest` + `JsonUtility` shadow DTOs). Auth shape: `IPlayerSession.GetAccessToken()` injected at construction; `HttpBackendClient` attaches `Authorization: Bearer <token>` on every call. Real login flow deferred. Request-side kind enum is a separate `QuestionKindFilter` (MC/TF/ANY) — engine's `QuestionKind` (MC/TF) stays a concrete tile property. 6/6 mock tests green; `HttpBackendClient` verified manually in C1.
**Why:** Pure interface in its own asmdef keeps `QuestionEngine` unit-testable without Unity — same rule that protects `GridEngine`. `IPlayerSession` named for what it operates (player auth context), not what implements it; one swap-point when real login lands. Separating filter from concrete kind preserves the invariant that a tile *is* either MC or TF — "ANY" is only meaningful in a request.
**Phase:** G2 / B2.

### 2026-05-26 — B3 shipped: QuestionFlow orchestrator (server-first, cancel-on-new, TF tier 3)
**Decision:** Pure-C# `QuestionFlow` (module `QuestionEngine`, class renamed to avoid the namespace-equals-class trap) wires `GridWorld` + `IBackendClient`. **Q1 — Resolve order: server-first.** `await client.ResolveAsync` runs before `engine.AttemptPuzzle`. **Q2 — Mid-flight clicks:** new `RequestQuestion` cancels the in-flight one via linked `CancellationTokenSource`; last click wins. **Q3 — TF cadence:** cap = `rand[3,7]` rolled once from the seed at construction; cadence interval = `rand[5,8]` per emission; TF questions always tier 3 regardless of player ratchet. Wildcards bypass TF entirely. **Bonus:** `CellCategory.White` is treated as a wildcard fetch alongside `IsWildcard==true` — unifies the original 2026-05-22 deterministic white category with B1's player-state re-skin. 9/9 tests green; 36/36 across all modules.
**Why:** Server-first preserves the audit-log invariant (server log and engine state never diverge); a brief UI delay on /resolve hiccups is the right tradeoff for prize-money trivia. Cancel-on-new respects "the player can select" — committing under mist pressure is itself the strategic moment, not the moment of clicking. TF cap as a random function (per-run from seed) means the cap itself can't be reverse-engineered into a farming pattern. Tier-3 TF is the "perceived breather" — easier than the player's likely ratchet position but still costs a wrong answer. White-as-wildcard collapses two parallel mechanisms (deterministic + re-skin) into a single fetch path; both produce identical server queries (`wildcard=true`).
**Phase:** G2 / B3.
