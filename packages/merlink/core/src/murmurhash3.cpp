...
// 128-bit x64 variant. Adapted for C++17 / merlink namespace.
...
inline uint64_t getblock64(const uint64_t* p, int i) {
    // Safe unaligned read via memcpy (optimized away by compilers)
    uint64_t val;
    __builtin_memcpy(&val, reinterpret_cast<const char*>(p) + i * 8, 8);
    return val;
}
