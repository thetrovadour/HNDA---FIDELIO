...
const CASHBACK_RATES: Record<string, string> = {
  TX_5:  '0.005', // 0.5%
  TX_10: '0.008', // 0.8%
  TX_25: '0.011', // 1.1%
...
const GOLD_MIN_SPEND = new Decimal('8000'); // L.8,000 per evaluation period
...
export class RewardService {
  ...
  private async countQualifyingSpends(
    user_id: string,
    since?: Date,
    until?: Date,
    excludeMerchantIds?: Set<string>,
  ): Promise<{ count: number; totalSpent: Decimal }> {
    ...
    for (const tx of spends) {
      if (excludeMerchantIds && tx.merchant_id && excludeMerchantIds.has(tx.merchant_id)) continue;
      ...
      if (soFar < MAX_DAILY_PER_MERCHANT) {
        ...
        count++;
        totalSpent = totalSpent.plus(new Decimal(tx.amount_catr));
      }
    }
  ...
  private getCurrentLookbackWindow(): { start: Date; end: Date } | null {
    const now = new Date();
  ...
  async isGoldActive(user_id: string): Promise<boolean> {
    const now = new Date();
  ...
  async getActiveTier(user_id: string): Promise<'GOLD' | 'TX_5' | 'TX_10' | 'TX_25' | null> {
    if (await this.isGoldActive(user_id)) return 'GOLD';
    const since = new Date(Date.now() - NINETY_DAYS_MS);
    const { count } = await this.countQualifyingSpends(user_id, since);
    if (count >= 25) return 'TX_25';
    if (count >= 10) return 'TX_10';
    if (count >= 5)  return 'TX_5';
    return null;
  }
  ...
  async evaluateGoldQualification(user_id: string): Promise<boolean> {
    ...
    const { count, totalSpent } = await this.countQualifyingSpends(
      user_id, window.start, window.end, ownedIds,
    );
  ...
  async processCashbackBatch(): Promise<{ processed: number; payouts: number }> {
    ...
    if (unprocessed.length === 0) return { processed: 0, payouts: 0 };

    const byUser = new Map<string, Array<{ id: string; amount_catr: any }>>();
    ...
    for (const [user_id, txns] of byUser) {
      if (await this.isGoldActive(user_id)) continue; // GOLD users handled hourly
      ...
      const totalSpend = txns.reduce(
        (sum, t) => sum.plus(new Decimal(t.amount_catr)),
        new Decimal(0),
      );
  ...
  async processGoldCashbackBatch(): Promise<{ processed: number; payouts: number }> {
    const unprocessed = await this.db.transaction.findMany({
      where: { type: 'SPEND', status: 'CONFIRMED', cashback_processed: false },
      select: { id: true, user_id: true, amount_catr: true },
    });
    ...
    for (const [user_id, txns] of byUser) {
      if (!await this.isGoldActive(user_id)) continue; // only GOLD users
  ...
  async checkCrossMerchantBonus(user_id: string): Promise<void> {
    ...
    if (visits.length >= 3) {
      ...
      if (!existing) {
        ...
        if (wallet) {
          const milestone = await this.db.rewardMilestone.create({
            data: { user_id, type: 'CROSS_MERCHANT', amount_catr: new Decimal('10') },
          });
          await this.queuePayout({
            recipient_wallet: wallet.address,
            amount_catr: '10',
