...
exports.initGcaAllocation = initGcaAllocation;
exports.evaluateGcaVesting = evaluateGcaVesting;
exports.currentPriceFloor = currentPriceFloor;
const library_1 = require("@prisma/client/runtime/library");
const GCA_GIFT = new library_1.Decimal('200');
const GCA_PER_MILESTONE = new library_1.Decimal('100');
const CATR_PER_MILESTONE = new library_1.Decimal('25000');
const MAX_MILESTONES = 10;
function uniqueClientMultiplier(ratio) {
    if (ratio >= 0.6)
        return new library_1.Decimal('2.0');
    if (ratio >= 0.4)
        return new library_1.Decimal('1.5');
    if (ratio >= 0.2)
        return new library_1.Decimal('1.25');
    return new library_1.Decimal('1.0');
}
async function initGcaAllocation(db, merchantId) {
...
async function evaluateGcaVesting(db, merchantId) {
    const allocation = await db.merchantGcaAllocation.findUnique({ where: { merchant_id: merchantId } });
    if (!allocation || allocation.milestones_claimed >= MAX_MILESTONES)
        return;
    ...
    const totalCatr = new library_1.Decimal(volumeAgg._sum.amount_catr ?? 0);
    ...
    const newMilestonesTotal = Math.min(Math.floor(effectiveCatr.div(CATR_PER_MILESTONE).toNumber()), MAX_MILESTONES);
...
async function currentPriceFloor(db) {
    const floor = await db.gcaPriceFloor.findFirst({ where: { active: true }, orderBy: { set_at: 'desc' } });
    return floor ? new library_1.Decimal(floor.price_hnl) : new library_1.Decimal('1');
}
