...
A closed-loop payment and loyalty network for Honduras, with the purpose of allowing the possibility of using mobile devices as POT (Point of Transactions). Built on Base (Ethereum L2).

---

## What it is

FIDELIO is a loyalty and payment system designed for small Honduran merchants. Clients load Lempiras into the network by using the Merchant's POT, receive CATR tokens 1:1 on HNL, spend them at participating merchants, and merchants redeem CATR back for Lempiras. Value circulates inside the Honduran economy instead of leaking out through foreign payment processors.

The CATR token is not a cryptocurrency to the end user. It is a loyalty point — that happens to live on a blockchain. The blockchain is the trust layer, not the product. This means that transparency is seen. You can see if the transaction ocurred, if there is enough for you to redeem, movements, etc.
...
Everything is seen in the Base Network. Nothing is missing.
```
...
Every CATR transfer charges 3.6% split in these percentages:
- 65% → HNDA treasury (operations, reserve, sustainability, etc)
...
Clients earn cashback as they build transaction history with the network. The rate is a milestone-based upgrade — the more you transact, the better your rate. Milestones stack. GOLD status (≥100 txns + ≥8,000 CATR spend in a 5-month window for example) locks in the best rate permanently and evaluates on Honduras's own labor calendar: June and December, the months workers already receive bonuses. FIDELIO takes advantage of these peculiar things in these seasons to better the experience.

### GCA — Guacacoin

GCA is the merchant loyalty token. It is not a payment instrument. It is equity in the network itself.

HNDA mints GCA to merchants based on their processed volume and the diversity of their client base. Merchants redeem GCA for HNL at the current price floor. The floor is calculated dynamically: `GCA reserve balance ÷ total GCA in circulation`. The reserve grows with every CATR transaction (0.351% of amount, from the treasury slice). The floor only moves up over time unless redeemed. 
...
High-value redemptions (>500 CATR) require VaultOp 2-of-2 authorization. This is the last line of defense against unauthorized large outflows
...
Right now, we are evaluating banking systems.
